...
class TemperBridgeComponent
  ...
  protected:
    ...
    void si446x_raw_command_(const uint8_t *tx_data, size_t tx_data_bytes,
                             uint8_t *resp, size_t resp_bytes);
    void si446x_execute_command_(uint8_t command, const uint8_t *args,
                                 size_t arg_bytes, uint8_t *data,
                                 size_t data_bytes);
    Si446xChipInfoResp si446x_part_info_();
    void si446x_configuration_init_(const uint8_t *data);
    void si446x_fifo_info_(Si446xFifoInfoResp *ret, bool clear_rx,
                           bool clear_tx);
    void si446x_start_tx_();
    void si446x_set_freq_control_properties_(uint8_t freq_control_inte,
                                             uint32_t freq_control_frac);
    void si446x_set_property_(Si446xSetPropertyArgs *args, uint8_t *data);
    void si446x_get_freq_control_properties_(uint8_t *freq_control_inte,
                                             uint32_t *freq_control_frac);
...
class ExecuteSimpleCommandAction : public Action<Ts...>,
                                   public Parented<TemperBridgeComponent> {
  public:
    TEMPLATABLE_VALUE(SimpleCommand, cmd);

    void play(Ts... x) override {
        this->parent_->execute_simple_command(this->cmd_.value(x...));
    }
};

template <typename... Ts>
class PositionCommandAction : public Action<Ts...>,
                              public Parented<TemperBridgeComponent> {
  public:
    TEMPLATABLE_VALUE(PositionCommand, cmd);

    void play(Ts... x) override {
        this->parent_->start_positioning(this->cmd_.value(x...));
    }
};

template <typename... Ts>
class SetChannelAction : public Action<Ts...>,
                         public Parented<TemperBridgeComponent> {
  public:
...
template <typename... Ts>
class SetMassageIntensityAction : public Action<Ts...>,
                                  public Parented<TemperBridgeComponent> {
  public:
...
template <typename... Ts>
class StartAutoLearnAction : public Action<Ts...>,
                             public Parented<TemperBridgeComponent> {
  public:
...
template <typename... Ts>
class StopAutoLearnAction : public Action<Ts...>,
                            public Parented<TemperBridgeComponent> {
  public:
