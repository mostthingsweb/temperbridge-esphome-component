...
const uint8_t SI4463_RADIO_CONFIGURATION_DATA_ARRAY[] =
...
void TemperBridgeComponent::si446x_raw_command_(const uint8_t *tx_data,
                                                size_t tx_data_bytes,
                                                uint8_t *resp,
                                                size_t resp_bytes) {
    // Wait for CTS
    while (true) {
...
Si446xChipInfoResp TemperBridgeComponent::si446x_part_info_() {
    ...
    this->si446x_execute_command_(SI446X_CMD_PART_INFO, nullptr, 0,
                                  (uint8_t *)&ret, sizeof(Si446xChipInfoResp));
...
void TemperBridgeComponent::si446x_execute_command_(uint8_t command,
                                                    const uint8_t *args,
                                                    size_t arg_bytes,
                                                    uint8_t *data,
                                                    size_t data_bytes) {
...
void TemperBridgeComponent::si446x_configuration_init_(const uint8_t *data) {
    while (*data != 0) {
        ...
        ESP_LOGI(TAG, "Processing command %x with # bytes: %d", command[0],
                 size_bytes);
        data += size_bytes;
...
void TemperBridgeComponent::si446x_get_int_status(Si446xGetIntStatusResp *ret,
                                                  bool clear_pending) {
    static_assert(sizeof(Si446xGetIntStatusResp) == 8, "wrong size");
    if (clear_pending) {
        si446x_execute_command_(SI446X_CMD_GET_INT_STATUS, nullptr, 0,
                                (uint8_t *)ret, sizeof(Si446xGetIntStatusResp));
    } else {
        const uint8_t args[] = {
        ...
        si446x_execute_command_(SI446X_CMD_GET_INT_STATUS, args, sizeof(args),
                                (uint8_t *)ret, sizeof(Si446xGetIntStatusResp));
    }
}
...
void TemperBridgeComponent::execute_simple_command(SimpleCommand cmd) {
    ...
    if (cmd == SimpleCommand::MASSAGE_PRESET_MODE1 ||
        cmd == SimpleCommand::MASSAGE_PRESET_MODE2 ||
        cmd == SimpleCommand::MASSAGE_PRESET_MODE3 ||
        cmd == SimpleCommand::MASSAGE_PRESET_MODE4) {
...
void TemperBridgeComponent::transmit_command_(uint32_t command) {
    ...
    TemperPacket packet = {.cmd = convert_big_endian(command),
                           .channel = convert_big_endian(this->channel_)};
...
void TemperBridgeComponent::si446x_fifo_info_(Si446xFifoInfoResp *ret,
                                              bool clear_rx, bool clear_tx) {
    static_assert(sizeof(Si446xFifoInfoResp) == 2, "");
    uint8_t const arg = (clear_tx ? (1 << 0) : 0) | (clear_rx ? (1 << 1) : 0);
    si446x_execute_command_(SI446X_CMD_FIFO_INFO, &arg, 1, (uint8_t *)ret,
                            sizeof(Si446xFifoInfoResp));
}
...
void TemperBridgeComponent::si446x_start_tx_() {
    ...
    si446x_execute_command_(SI446X_CMD_START_TX, tx_args, sizeof(tx_args),
                            nullptr, 0);
}
...
inline float temper_frequency_for_channel(uint16_t channel) {
    ...
    const uint16_t fc =
        channel > 8862 ? ((2 * channel) + 10658) : channel + 19520;
    return 10.0f * (19 + 24 + (fc / 64000.0f));
}

// TODO these methods should go in the si446x file
void temper_calculate_freq_control(uint16_t channel, uint8_t *freq_control_inte,
...
void TemperBridgeComponent::si446x_set_freq_control_properties_(
    uint8_t freq_control_inte, uint32_t freq_control_frac) {
    ...
    uint8_t data[] = {
        freq_control_inte,
        static_cast<uint8_t>((freq_control_frac & 0xFFFF00) >> 16),
        static_cast<uint8_t>((freq_control_frac & 0xFF00) >> 8),
        static_cast<uint8_t>(freq_control_frac & 0xFF)};
    si446x_set_property_(&args, data);
}

void TemperBridgeComponent::si446x_set_property_(Si446xSetPropertyArgs *args,
                                                 uint8_t *data) {
    ...
    si446x_execute_command_(SI446X_CMD_SET_PROPERTY, full_args,
                            sizeof(full_args), &cts, 1);
}

void TemperBridgeComponent::si446x_get_property_(Si446xGetPropertyArgs *args,
                                                 uint8_t *props) {
    static_assert(sizeof(Si446xGetPropertyArgs) == 3, "wrong size");
    uint8_t resp[args->num_props];
    si446x_execute_command_(SI446X_CMD_GET_PROPERTY, (uint8_t *)args,
                            sizeof(Si446xGetPropertyArgs), (uint8_t *)resp,
                            args->num_props);
    memcpy(props, resp, args->num_props);
}

void TemperBridgeComponent::si446x_get_freq_control_properties_(
    uint8_t *freq_control_inte, uint32_t *freq_control_frac) {
    ...
    *freq_control_frac =
        freq_props[3] | (freq_props[2] << 8) | (freq_props[1] << 16);
}
...
void TemperBridgeComponent::set_massage_level(MassageTarget target,
                                              uint8_t level) {
    uint32_t command =
        this->massage_command_mode_ == MassageCommandMode::BUILTIN
            ? TEMPER_MASSAGE_MAGIC_1
            : TEMPER_MASSAGE_MAGIC_2;
